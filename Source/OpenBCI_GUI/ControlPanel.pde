...
class ControlPanel {
    ...
    ADS129xConfigBox ads129xConfigBox;
    BLEBox bleBox;
    ...
    ControlPanel(OpenBCI_GUI mainClass) {
        ...
        ads129xConfigBox = new ADS129xConfigBox(x + w, dataSourceBox.y, w, h, globalPadding);
    ...
    public void draw() {
        ...
        if (systemMode != 10) { // only draw control panel boxes if system running is false
            ...
            } else if (eegDataSource == DATASOURCE_ADS129X) {
                ads129xConfigBox.draw();
            }
        }
...
class DataSourceBox {
    ...
    private void createDatasourceList(ControlP5 _cp5, String name, int _x, int _y, int _w, int _h, PFont font) {
        ...
        sourceList.addItem("ADS1299 16ch (WiFi TCP)", DATASOURCE_ADS129X);
            ...
            public void controlEvent(CallbackEvent theEvent) {
                if (theEvent.getAction() == ControlP5.ACTION_BROADCAST) {
                    ...
                    if (eegDataSource == DATASOURCE_CYTON) {
                        ...
                        updateToNChan(16);
                        controlPanel.ads129xConfigBox.setSampleRate(250);
                    } else if (eegDataSource == DATASOURCE_SYNTHETIC) {
...
class ADS129xConfigBox {
    public int x, y, w, h, padding;
    private final String boxLabel = "ADS1299 16CH WIFI TCP";
    private final String portLabel = "LISTEN PORT";
    private final String sampleRateLabel = "SAMPLE RATE";
    private final int titleH = 22;
    private final int labelH = 14;
    private final int objectH = 24;
    private final int hintH = 30;
    private ControlP5 localCP5;
    ...
    ADS129xConfigBox(int _x, int _y, int _w, int _h, int _padding) {
        ...
        w = _w - _padding;
        padding = _padding;
        h = titleH + labelH*2 + objectH*2 + hintH + padding*7;

        localCP5 = new ControlP5(ourApplet);
        localCP5.setGraphics(ourApplet, 0,0);
        localCP5.setAutoDraw(false);

        int portY = y + padding*2 + titleH + labelH;
        listenPort = localCP5.addTextfield("ads129xListenPort")
            .setPosition(x + padding, portY)
            .setCaptionLabel("")
            .setSize(96, objectH)
            .setFont(f2)
            .setFocus(false)
            ...
            .setColorForeground(OPENBCI_DARKBLUE)
    ...
    public void update() {
        copyPaste.checkForCopyPaste(listenPort);
    }
    ...
    public void draw() {
        ...
        text(boxLabel, x + padding, y + padding);
        textFont(p4, 14);
        int portLabelY = y + padding*2 + titleH;
        int portY = portLabelY + labelH;
        int sampleRateLabelY = portY + objectH + padding;
        int buttonY = sampleRateLabelY + labelH;
        int hintY = buttonY + objectH + padding*2;
        text(portLabel, x + padding, portLabelY);
        text(sampleRateLabel, x + padding, sampleRateLabelY);
        textFont(p5, 12);
        text("Set board AT+IP to this PC IPv4.", x + padding, hintY);
        text("Set AT+PORT to the listen port.", x + padding, hintY + 13);
        popStyle();

        localCP5.draw();
    }

    private Button createSampleRateButton(String name, String text, int _x, int _y, int _w, int _h, final int rate) {
        final Button button = createButton(localCP5, name, text, _x, _y, _w, _h);
        button.setSwitch(true);
        button.onRelease(new CallbackListener() {
            public void controlEvent(CallbackEvent theEvent) {
                setSampleRate(rate);
                println("ControlPanel: User selected ADS1299 " + rate + "Hz");
            }
        });
        return button;
    }
    ...
    public void setSampleRate(int rate) {
        setSampleRateInternal(rate, true);
    }
    ...
    public int getListenPort() {
        try {
            return Integer.parseInt(listenPort.getText().trim());
        } catch (Exception e) {
            return -1;
        }
    }

    public boolean hasValidListenPort() {
    ...
    public void initButtonPressed() {
        if (getInitSessionButtonText().equals("START SESSION")) {
            if ((eegDataSource == DATASOURCE_CYTON && selectedProtocol == BoardProtocol.NONE) || (eegDataSource == DATASOURCE_GANGLION && selectedProtocol == BoardProtocol.NONE)) {
                ...
                outputWarn("No Wifi Shield selected. Please select your Wifi Shield and retry system initiation.");
                return;
            } else if (eegDataSource == DATASOURCE_ADS129X && !controlPanel.ads129xConfigBox.hasValidListenPort()) {
                outputWarn("Invalid ADS1299 listen port. Enter a TCP port from 1 to 65535.");
                return;
